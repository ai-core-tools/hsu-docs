...
py-info:
	...
	@echo "pyproject.toml: $(if $(wildcard $(PYTHON_DIR)/pyproject.toml),✓ Found,❌ Missing)"
	@echo "requirements.txt: $(if $(wildcard $(PYTHON_DIR)/requirements.txt),✓ Found,❌ Missing)"
	@echo "setup.py: $(if $(wildcard $(PYTHON_DIR)/setup.py),✓ Found,❌ Missing)"

# Run targets (if executables exist)
.PHONY: py-run-cli py-run-srv

## Python Run CLI - Run the first CLI found
py-run-cli:
ifneq ($(word 1,$(PY_CLI_TARGETS)),)
	@echo "Running CLI: $(word 1,$(PY_CLI_TARGETS))"
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry run python $(subst $(PYTHON_DIR)/,,$(word 1,$(PY_CLI_TARGETS)))
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm run python $(subst $(PYTHON_DIR)/,,$(word 1,$(PY_CLI_TARGETS)))
else
	$(PY_CMD) python $(subst $(PYTHON_DIR)/,,$(word 1,$(PY_CLI_TARGETS)))
endif
else
	@echo "No CLI targets found. Check $(PYTHON_DIR)/$(PY_CLI_BUILD_DIR) directory."
endif

## Python Run Server - Run the first server found
py-run-srv:
ifneq ($(word 1,$(PY_SRV_TARGETS)),)
	@echo "Running server: $(word 1,$(PY_SRV_TARGETS))"
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry run python $(subst $(PYTHON_DIR)/,,$(word 1,$(PY_SRV_TARGETS))) --port $(DEFAULT_PORT)
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm run python $(subst $(PYTHON_DIR)/,,$(word 1,$(PY_SRV_TARGETS))) --port $(DEFAULT_PORT)
else
	$(PY_CMD) python $(subst $(PYTHON_DIR)/,,$(word 1,$(PY_SRV_TARGETS))) --port $(DEFAULT_PORT)
endif
else
	@echo "No server targets found. Check $(PYTHON_DIR)/$(PY_SRV_BUILD_DIR) directory."
endif

# Development helpers
.PHONY: py-watch py-shell py-repl py-jupyter

## Python Watch - Watch for changes and run tests (requires entr)
py-watch:
ifeq ($(shell which entr 2>$(NULL_DEV)),)
	...
	@find $(PYTHON_DIR) -name "*.py" | entr -r make py-test
endif
...
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	...
	$(PY_CMD) pdm run python
else
	$(PY_CMD) python
endif

## Python REPL - Open interactive Python REPL
py-repl:
	@echo "Opening Python REPL..."
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry run python -i
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm run python -i
else
	$(PY_CMD) python -i
endif

## Python Jupyter - Start Jupyter notebook (if available)
py-jupyter:
ifeq ($(shell which jupyter 2>$(NULL_DEV)),)
	@echo "Jupyter not found. Install with: pip install jupyter"
else
	@echo "Starting Jupyter notebook..."
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry run jupyter notebook
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm run jupyter notebook
else
	$(PY_CMD) jupyter notebook
endif
endif

# Package management
.PHONY: py-update py-freeze py-security

## Python Update - Update dependencies
py-update:
	@echo "Updating Python dependencies..."
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry update
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm update
else
	$(PY_CMD) pip install --upgrade -r requirements.txt
endif

## Python Freeze - Generate requirements.txt
py-freeze:
	@echo "Generating requirements.txt..."
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry export -f requirements.txt --output requirements.txt
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm export -f requirements.txt --output requirements.txt
else
	$(PY_CMD) pip freeze > requirements.txt
endif

## Python Security - Check for security vulnerabilities
py-security:
	@echo "Checking for security vulnerabilities..."
ifeq ($(shell which safety 2>$(NULL_DEV)),)
	@echo "safety not found. Install with: pip install safety"
else
ifeq ($(PYTHON_BUILD_TOOL),poetry)
	$(PY_CMD) poetry run safety check
else ifeq ($(PYTHON_BUILD_TOOL),pdm)
	$(PY_CMD) pdm run safety check
else
	$(PY_CMD) safety check
endif
endif 